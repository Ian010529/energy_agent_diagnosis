...
class CaseApplicationService:
    ...
    async def review_diagnosis(
        ...
        review = await self.review_recorder.append(
            AppendCaseReviewInput.model_validate(
                {
                    "context": {
                        "trace_id": trace_id,
        ...
        if self.timeline:
            await self.timeline.append(
                session_id,
                TimelineEventType.REVIEW_SUBMITTED,
                review_id,
                run_id=result.run_id,
                actor=actor,
                payload={
    ...
    async def submit(
        ...
        transition = await self.cases.transition(
            case_id,
        ...
        await self._audit_case(actor, "case.submitted", transition.case)
        return transition.case
