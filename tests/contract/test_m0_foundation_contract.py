...
def test_integration_target_runs_the_persistent_m0_gate() -> None:
    ...
    assert '"deployment_profiles": verified_profiles' in gate
    assert 'etcdctl", "put"' in gate
    assert '"--print-value-only"' in gate
    assert 'f"{base_project}-full-{acceptance_run_id[:8]}"' in gate
    assert '"COMPOSE_PROJECT_NAME": f"{base_project}-{profile}-' in gate
    assert '"profile_projects": profile_projects' in gate
    assert 'signal.signal(signum, stop_on_signal)' in gate
    assert '"git", "status", "--porcelain", "--untracked-files=all"' in gate
