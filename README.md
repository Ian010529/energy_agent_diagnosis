...
本目录当前实现全局阶段 1 工程底座与阶段 2 数据接入 Mock 基线：虚拟环境、模块化 FastAPI
工程骨架、统一契约、Provider 抽象、配置、日志、Trace、健康检查、基础鉴权、监控指标、CI
基线，以及设备画像、告警详情、时序摘要、手册 chunk 和历史工单五类只读 Mock Provider。
...
当前可执行能力包含阶段 1 工程底座，以及阶段 2 数据接入 Mock 基线。阶段 2 已覆盖设备画像、
告警详情、时序摘要、手册 chunk 和历史工单五类只读 Provider，并保留 graph、工单写入和案例
审核 Provider 的 Null 边界。

当前实现不包含查询重写、混合召回、reranker、证据归并、LangGraph 主流程、候选根因生成、
Redis 会话记忆、案例审核闭环、Web 前端或真实外部系统 Adapter。
