...
> **文档定位（2026-07-18 治理重置）**：本文保留为试点前硬化和生产发布阶段的
> 可靠性、安全、并发、部署与验收参考，不再作为当前单人开发的逐模块阻塞流程。
> 当前开发权威顺序、执行方式和阶段状态分别以根目录 `AGENTS.md` 与
> `docs/IMPLEMENTATION_STATUS.yaml` 为准；当前阶段不要求 M0 → M11 串行，也不
> 要求普通修改执行 full、live、chaos 或 performance Gate。
